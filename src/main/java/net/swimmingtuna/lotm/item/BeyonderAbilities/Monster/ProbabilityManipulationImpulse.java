...
public class ProbabilityManipulationImpulse extends LeftClickHandlerSkillP {
    ...
    public static void probabilityImpulse(LivingEntity player) {
        ...
        if (!level.isClientSide()) {
            for (Player pPlayer : level.players()) {
                for (LivingEntity livingEntity : pPlayer.level().getEntitiesOfClass(LivingEntity.class, pPlayer.getBoundingBox().inflate(BeyonderUtil.getDamage(player).get(ItemInit.PROBABILITYEFFECT.get())))) {
    ...
    public <T> LeftClickType getleftClickEmpty(T item) {
        return new UpdateItemInHandC2S((Integer) item, new ItemStack(ItemInit.PROBABILITYFORTUNE.get()));
    }
}
