...
public class WitherRune extends Item {

    public WitherRune(Properties pProperties) {
        super(pProperties);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pHand) {
        ...
        if (!pLevel.isClientSide() && pPlayer.getMainHandItem().getItem() instanceof WitherRune) {
            if (!pPlayer.isShiftKeyDown()) {
                ItemStack offStack = pPlayer.getItemInHand(InteractionHand.OFF_HAND);
                Enchantment witherEnchant = EnchantmentInit.WITHER_ENCHANTMENT.get();
                if (!offStack.isEmpty() && witherEnchant.canEnchant(offStack)) {
                    Map<Enchantment, Integer> enchantments = EnchantmentHelper.getEnchantments(offStack);
                    enchantments.remove(witherEnchant);
                    enchantments.put(witherEnchant, 50);
