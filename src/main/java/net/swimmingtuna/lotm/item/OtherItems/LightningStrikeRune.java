...
public class LightningStrikeRune extends Item {

    public LightningStrikeRune(Properties pProperties) {
        super(pProperties);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pHand) {
        ...
        if (!pLevel.isClientSide() && pPlayer.getMainHandItem().getItem() instanceof LightningStrikeRune) {
            if (!pPlayer.isShiftKeyDown()) {
                ItemStack offStack = pPlayer.getItemInHand(InteractionHand.OFF_HAND);
                Enchantment lightningEnchant = EnchantmentInit.LIGHTNING_STRIKE.get();
                if (!offStack.isEmpty() && lightningEnchant.canEnchant(offStack)) {
                    Map<Enchantment, Integer> enchantments = EnchantmentHelper.getEnchantments(offStack);
                    enchantments.remove(lightningEnchant);
                    enchantments.put(lightningEnchant, 50);
