...
public class FlameRune extends Item {

    public FlameRune(Properties pProperties) {
        super(pProperties);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pHand) {
        ...
        if (!pLevel.isClientSide() && pPlayer.getMainHandItem().getItem() instanceof FlameRune) {
            if (!pPlayer.isShiftKeyDown()) {
                ItemStack offStack = pPlayer.getItemInHand(InteractionHand.OFF_HAND);
                Enchantment flame = EnchantmentInit.FLAME.get();
                if (!offStack.isEmpty() && flame.canEnchant(offStack)) {
                    Map<Enchantment, Integer> enchantments = EnchantmentHelper.getEnchantments(offStack);
                    enchantments.remove(flame);
